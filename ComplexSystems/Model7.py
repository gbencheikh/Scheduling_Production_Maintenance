# -*- coding: utf-8 -*-
"""
Created on Thu Nov  6 17:52:12 2025
...
class FJSP_Maintenance_Quality_complex_systems__model:
    ...
    def solve(self):
        ...
        # Sequencing adjacency on each machine: w[n,k,jp,ip,j,i] = AND(x[n,k,jp,ip], x[n+1,k,j,i])
        w = {}
        for n in N:
            if n == N[-1]:  # no successor slot at the last n
                continue
            for k in K:
                for jp in J:
                    for ip in I[jp]:
                        if not can_run[k][jp][ip]:
                            continue
                        for j in J:
                            for i in I[j]:
                                if not can_run[k][j][i]:
                                    continue
                                w[n,k,jp,ip,j,i] = model.addVar(vtype=GRB.BINARY,name=f"w[{n},{k},{jp},{ip}->{j},{i}]")

        # z[n,k,l,jp,ip,j,i] = AND(w[n,k,jp,ip,j,i], y[n,k,l])  (maintenance between two ops)
        z = {}
        for key in w:
            n,k,jp,ip,j,i = key
            for l in L[k]:
                z[n,k,l,jp,ip,j,i] = model.addVar(vtype=GRB.BINARY, name=f"z[{n},{k},{l},{jp},{ip}->{j},{i}]")
        ...
        # pi_wt[n,k,jp,ip,j,i] = w * t[jp,ip]
        pi_wt = {key: model.addVar(lb=0.0, name=f"pi_wt[{key}]") for key in w}
        ...
        for k in K:
            for n in N:
                ...
                model.addConstr(v[n,k] == qsum(x[n,k,j,i] for j in J for i in I[j]), name=f"Cstr-A0_{k}{n}")
            for n in N[:-1]:
        ...
        for j in J:
            for i in I[j]:
                model.addConstr(qsum(x[n,k,j,i] for n in N for k in K) == 1, name=f"Cstr-B_{j}{i}")
        ...
        for n in N:
            for k in K:
                for l in L[k]:
                    model.addConstr(
                        d[n,k,l] == qsum(x[n,k,j,i] * p[k][j][i] * delta[k][l][j][i] for j in J for i in I[j]), name=f"Cstr-C_{j}{i}"
                    )
        ...
        for k in K:
            for l in L[k]:
                ...
                """
                model.addConstr( E[0,k,l] == qsum(x[0,k,j,i]*p[k][j][i]*delta[k][l][j][i] for j in J for i in I[j]), name=f"Cstr-D01_{k}{l}")
                model.addConstr( D[0,k,l] <= E[0,k,l],                                                               name=f"Cstr-D02_{k}{l}")
                ...
                for n in range(1,self.n_max):
                    model.addConstr( E[n,k,l] == D[n-1,k,l] - pi_yD[n-1,k,l] + qsum(x[n,k,j,i]*p[k][j][i]*delta[k][l][j][i] for j in J for i in I[j]), name=f"Cstr-D08_{k}{l}{n}" )
                for n in range(self.n_max):
                    
                    model.addConstr( D[n,k,l] <= E[n,k,l],                                                               name=f"Cstr-D09_{k}{l}{n}")
        ...
        # F) Adjacency w = AND(x[n,k,jp,ip], x[n+1,k,j,i])
        for key in w:
            n,k,jp,ip,j,i = key
            model.addConstr(w[key] <= x[n,k,jp,ip],                     name=f"Cstr-F0_{n}{k}{jp}{ip}{j}{i}")
            model.addConstr(w[key] <= x[n+1,k,j,i],                     name=f"Cstr-F1_{n}{k}{jp}{ip}{j}{i}")
            model.addConstr(w[key] >= x[n,k,jp,ip] + x[n+1,k,j,i] - 1,  name=f"Cstr-F2_{n}{k}{jp}{ip}{j}{i}")

        # G) z = AND(w, y[n,k,l])
        for key in z:
            n,k,l,jp,ip,j,i = key
            model.addConstr(z[key] <= w[n,k,jp,ip,j,i],                 name=f"Cstr-G0_{n}{k}{l}{jp}{ip}{j}{i}")
            model.addConstr(z[key] <= y[n,k,l],                         name=f"Cstr-G1_{n}{k}{l}{jp}{ip}{j}{i}")
            model.addConstr(z[key] >= w[n,k,jp,ip,j,i] + y[n,k,l] - 1,  name=f"Cstr-G2_{n}{k}{l}{jp}{ip}{j}{i}")

        # H) pi_wt = w * t[jp,ip]
        for key in w:
            n,k,jp,ip,j,i = key
            model.addConstr(pi_wt[key] >= 0,                                name=f"Cstr-H0_{n}{k}{jp}{ip}{j}{i}")
            model.addConstr(pi_wt[key] <= tmax * w[key],                    name=f"Cstr-H1_{n}{k}{jp}{ip}{j}{i}")
            model.addConstr(pi_wt[key] <= t[jp,ip],                         name=f"Cstr-H2_{n}{k}{jp}{ip}{j}{i}")
            model.addConstr(pi_wt[key] >= t[jp,ip] - tmax * (1 - w[key]),   name=f"Cstr-H3_{n}{k}{jp}{ip}{j}{i}")
        ...
        for j in J:
            for i in I[j][:-1]:
                ...
                model.addConstr(
                    t[j,i+1] >= t[j,i] +
                    qsum(x[n,k,j,i] * p[k][j][i] for n in N for k in K if (n,k,j,i) in x), name=f"Cstr-I_{j}{i}"
                )

        # J) Non-overlap lower bound for each operation start (build once per (j,i))
        for j in J:
            for i in I[j]:
                LB = qsum(pi_wt[n,k,jp,ip,j,i] + w[n,k,jp,ip,j,i] * p[k][jp][ip] for (n,k,jp,ip) in x.keys() if (n,k,jp,ip,j,i) in w.keys())
                # Add maintenance durations between consecutive ops (sum over l on same machine)
                LB += qsum(z[n,k,l,jp,ip,j,i] * m_kl[k][l] for (n,k,l) in y for (jp,ip) in t if (n,k,l,jp,ip,j,i) in z)
                model.addConstr(t[j,i] >= LB,                               name=f"Cstr-J0_{j}{i}")
                model.addConstr(qsum(x[n,k,j,i] for n in N for k in K)==1 , name=f"Cstr-J1_{j}{i}")
        ...
        if model.status == GRB.OPTIMAL or model.status == GRB.TIME_LIMIT:
            ...
            schedule.sort()
            for rec in schedule:
                print("Machine %d, slot %d: job %d op %d starts at %.2f (dur=%.2f)" % rec)
...
if __name__ == "__main__": 
    ...
    aql=0.9          # acceptable quality level triggering quality penality ()
