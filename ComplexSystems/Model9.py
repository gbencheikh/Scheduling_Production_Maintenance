# -*- coding: utf-8 -*-
"""
Created on Sat Dec 13 14:34:35 2025
...
class FJSP_Maintenance_Quality_complex_systems__model:
    ...
    def solve(self):
        ...
        M_D = 1000 # Big M pour la dégradation
        
        for j1, i1, k, l in D_indices: # Pour chaque op (j',i') sur machine k, composant l
            # 1. Initialisation D (Première opération sur la machine k)
            # La dégradation D avant la première op est 0 si aucune maintenance y=0.
            # D[j',i',k,l] <= (1-y) * D_max
            #model.addConstr(D[j1,i1,k,l] <= M_D * gp.quicksum(x[j,i,k] for j,i in all_ops), name=f"Cstr-D0_M_{j1}{i1}{k}{l}") # D est 0 si la machine n'est pas utilisée
            # D'abord, le reset de maintenance
            #model.addConstr(D[j1,i1,k,l] <= M_D * (1 - y[j1,i1,k,l]) ,                      name=f"Cstr-D1_RST_{j1}{i1}{k}{l}")
    
            # 2. Récurrence (D avant j'i' est D après j i, si j i précède j'i')
            # D_after[j,i,k,l] = D[j,i,k,l] + d[j,i,k,l] (d=new degradation)
            # D_after[j,i,k,l] = D[j,i,k,l] + x[j,i,k] * p[k][j][i] * delta[k][l][j][i]
            # D[j',i',k,l] >= D_after[j,i,k,l] - M * (1 - z[j,i,j',i',k]) - M * y[j',i',k,l]
            for j, i in all_ops:
                if (j1, i1, j, i, k) in z:
                    # calcul de D_before
                    model.addConstr(D_before[j1,i1,k,l] <= M_D*(2-y[j,i,k,l]-x[j1,i1,k]) ,                          name=f"Cstr-D21_{j1}{i1}{j}{i}{k}{l}")
                    #model.addConstr(D_before[j1,i1,k,l] >= p[k][j][i] * delta[k][l][j][i] * ybarZ[j,i,j1,i1,k,l],  name=f"Cstr-D22_{j1}{i1}{j}{i}{k}{l}")
                    model.addConstr(D_before[j1,i1,k,l] <= D[j,i,k,l] + M_D * (1-z[j,i,j1,i1,k]),                   name=f"Cstr-D23_{j1}{i1}{j}{i}{k}{l}")
                    model.addConstr(D_before[j1,i1,k,l] >= D[j,i,k,l] - M_D * (1-z[j,i,j1,i1,k]+y[j,i,k,l]),        name=f"Cstr-D24_{j1}{i1}{j}{i}{k}{l}")
                    """
                    # Calcul de D_after
                    D_after_ji = D[j,i,k,l] + x[j,i,k] * p[k][j][i] * delta[k][l][j][i]
                    # Récurrence si (j,i) précède (j',i') et PAS de maintenance
                    model.addConstr(D[j1,i1,k,l] >= D_after_ji - M_D * (1 - z[j,i,j1,i1,k]) - M_D * y[j1,i1,k,l], name=f"Cstr-D2_REC_{j}{i}_{j1}{i1}{k}{l}")
                    """
            model.addConstr(D[j1,i1,k,l] >= D_before[j1,i1,k,l] + p[k][j1][i1] * delta[k][l][j1][i1]-M_D*(1-x[j1,i1,k]), name=f"Cstr-D31_{j1}{i1}{k}{l}")
            model.addConstr(D[j1,i1,k,l] <= D_before[j1,i1,k,l] + p[k][j1][i1] * delta[k][l][j1][i1]+M_D*(1-x[j1,i1,k]), name=f"Cstr-D32_{j1}{i1}{k}{l}")
            # 3. Lien D et x (Si x[j',i',k]=0, D[j',i',k,l] n'a pas besoin d'être défini)
            # En pratique, si x=0, D doit être 0
            # D[j',i',k,l] <= Dmax * x[j',i',k]
