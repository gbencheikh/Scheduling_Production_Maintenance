        ...
        for k in self.K:
            for r in self.R[k]:
                for l in self.L[k]:
                    ...
                    model.addConstr(
                        self.D_after[k, l, r] == self.D_before[k, l, r] + Inc,
                        name=f"Dacc[{k},{l},{r}]"
                    )
                    # Slot vide → D_after = 0
                    model.addConstr(
                        self.D_after[k, l, r] <= self.Dmax_kl[k, l] * self.U[k, r],
                        name=f"Dacc_empty[{k},{l},{r}]"
                    )
        return model
...
def print_solution(data, optsolution, Cmax, nbrmaint, avgoq, qualpenal):
    seq_jobs, seq_machs, y, t_matrix, rank = optsolution

    # Reconstruire i_s (index local de chaque op dans son job)
    ...
    print(f"  Maintenances: {int(nbrmaint)}")
    print(f"  Avg Quality : {avgoq:.4f}")
    print(f"  Qual. Penal : {qualpenal:.4f}")
    print("="*65)

    # ── Séquence par machine ──────────────────────────────────────
    print("\n── Séquence par machine ──")
    ...
    for ind in range(nb_ops_total):
        ...
        i = i_s[ind]
        k = seq_machs[ind]
        t_start = t_matrix[j][i]
        p       = data.dureeOperations[k][j][i]
        t_end   = t_start + p
        ops_per_machine[k].append((t_start, t_end, j, i, p))

    for k in range(data.nbMachines):
        ops = sorted(ops_per_machine[k], key=lambda x: x[0])
        print(f"\n  Machine {k+1}:")
        print(f"    {'Op':8s} {'Start':>7s} {'End':>7s} {'Dur':>5s} {'Maint après?':>14s}")
        print(f"    {'-'*8} {'-'*7} {'-'*7} {'-'*5} {'-'*14}")
        for (t_start, t_end, j, i, p) in ops:
            maints = []
            for l in range(data.nbComposants[k]):
                if y[l][j][i]:
                    maints.append(f"comp{l}(dur={data.dureeMaintenances[k][l]})")
            maint_str = ", ".join(maints) if maints else "-"
            print(f"    J{j+1}O{i+1}     {t_start:>7.2f} {t_end:>7.2f} {p:>5.2f} {maint_str:>14s}")

    # ── Séquence par job ─────────────────────────────────────────
    print("\n── Séquence par job ──")
    for j in range(data.nbJobs):
        print(f"\n  Job {j+1}:")
        print(f"    {'Op':8s} {'Machine':>8s} {'Start':>7s} {'End':>7s} {'Dur':>5s}")
        print(f"    {'-'*8} {'-'*8} {'-'*7} {'-'*7} {'-'*5}")
        for i in range(data.nbOperationsParJob[j]):
            # Retrouver la machine de (j,i)
            k = next(seq_machs[ind] for ind in range(nb_ops_total)
                     if seq_jobs[ind] == j and i_s[ind] == i)
            t_start = t_matrix[j][i]
            p       = data.dureeOperations[k][j][i]
            t_end   = t_start + p
            print(f"    O{i+1}       M{k+1:>6}  {t_start:>7.2f} {t_end:>7.2f} {p:>5.2f}")

    # ── Récapitulatif maintenances ────────────────────────────────
    print("\n── Maintenances déclenchées ──")
    found = False
    for ind in range(nb_ops_total):
        j = seq_jobs[ind]
        i = i_s[ind]
        ...
        for l in range(data.nbComposants[k]):
            if y[l][j][i]:
                ...
                p       = data.dureeOperations[k][j][i]
                t_maint = t_start + p
                dur_m   = data.dureeMaintenances[k][l]
                print(f"  Après J{j+1}O{i+1} sur M{k+1} | composant {l} | "
                      f"début maint={t_maint:.2f} durée={dur_m} fin={t_maint+dur_m:.2f}")
    if not found:
        print("  Aucune maintenance.")
...
if __name__ == "__main__": 
    ...
    alphakl=0           # quality degradation rate
    ...
    print_solution(data, optsolution1, optCmax1, nbrmaint1, avgoq1, qualpenal1)
